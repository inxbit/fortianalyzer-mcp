...
### Fixed
- **Log search poll path rebuilt on the documented API (fixes #27, the FAZ 8.0 breakage).** The page runner polled `GET /logview/adom/{adom}/logsearch/count/{tid}` and read `progress-percent`/`total-logs`/`scanned-logs` — an endpoint and field names that are not in the documented API (they happened to behave on 7.6.7 and broke against 8.0 GA with cascading `search_timeout` → slot exhaustion). The runner now polls the single documented read endpoint `GET /logview/adom/{adom}/logsearch/{tid}` and uses spec `percentage >= 100` as the only readiness signal; sub-100 responses (which can carry partial data) are discarded, never returned. Live-verified on 7.6.7: the fetch GET is repeatable while the scan runs, and a completed task is NOT reaped by fetching it — so the runner now always best-effort cancels the tid on exit (success included) to keep the appliance search-slot pool drained. `FortiAnalyzerClient.logsearch_count()` is removed.
