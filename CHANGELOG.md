...
### Fixed
- **Log search restored on FortiAnalyzer 7.6.7.** After upgrading from 7.4.x to 7.6.7, the log-search tools (`query_logs`, the `search_*` helpers, and the policy-analysis tools) regressed to zero rows and intermittent `Not connected` / `Invalid JSON response` / `search_timeout` errors, while FortiView kept returning traffic from the same appliance. Four independent defects were addressed:
  - **Single client lifecycle owner.** In HTTP mode the FastMCP `stateless_http` lifespan ran per request/session and connected *and disconnected* the process-global client around each call, dropping the session out from under concurrent requests. The shared `faz_client` is now owned by exactly one path (`app_lifespan` in HTTP mode, `run_stdio` in stdio mode); the per-session lifespan no longer touches it.
  - **Reconnect on a dropped session mid-request.** A raw `ConnectionError("Not connected. ...")` raised after the session dropped is now treated as recoverable and triggers one forced re-login + retry — but only for a client that was previously connected (a never-connected client still surfaces the raw error). Concurrent reconnects are serialized by a lock + generation counter so a single re-login is shared, not raced.
  - **No more false `count:0`.** A `percentage:100` fetch with an *empty* `data` page while `total-count` reports rows at/after the offset (observed on 7.6.7) is no longer treated as a completed empty result; the search is re-issued a bounded number of times before the page is accepted, so a transient empty completion settles into real rows.
  - **LogView clock alignment.** Relative windows (`1-hour`, `7-day`, …) are now anchored on the appliance's latest LogView ingest time rather than the FAZ system-clock "now", so a post-upgrade clock skew no longer places the window ahead of the newest ingested log.
...
- **Time-basis metadata.** `query_logs`, `fetch_more_logs`, and the three policy-analysis tools report `time_basis_source` (`logfiles_state` | `logstats` | `faz_tz` | `naive` | `custom`) — which clock a relative window was anchored on — and `clock_skew_seconds` (the detected LogView-vs-system clock offset, when measurable).
- **Unified device-filter construction.** The log, traffic, and pcap tools now build the FAZ `device` filter through a single shared `build_device_filter` helper (previously three near-duplicate copies plus inline pcap forms), so serial/`All_*`/device-name handling is identical everywhere.
