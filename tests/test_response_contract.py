...
class ContractFaz:
    ...
    Models an immediately-complete scan: every ``logsearch_fetch`` reports
    ``percentage: 100`` with the page, so the runner's first fetch poll
    delivers (spec contract per upstream issue #27).
    """
